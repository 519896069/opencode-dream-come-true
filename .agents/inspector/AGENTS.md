...
- 单元测试质量：测试代码是否有意义
- 路由/页面一致性：代码是否遵循 design.md 中的路由/页面定义
...
### I5. 路由/页面一致性审查红线
前端代码必须满足以下条件，否则判定为 fail：
- 同一个需求必须在同一个路由/页面中完成
- 禁止将一个需求拆分到多个页面
- 如果 design.md 指定在现有页面中修改，禁止擅自新建页面
- 路由/页面必须与 design.md 中的定义一致
...
3. 按维度审查：
   ...
   f. 路由/页面一致性：检查是否遵循 design.md 的路由/页面定义（仅前端任务）
4. 生成审查报告
5. 返回审查结果
```
